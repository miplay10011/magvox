// magic.js – исправленная версия с поддержкой мобов и без ошибок
...
export function createMagicEngine(ctx) {
  ...
  // Вспомогательная функция для получения эффектов игрока/сущности
  function getEffectsForId(id) {
    const players = ctx.getPlayers();
    for (const [pid, p] of players) {
      if (pid === id) return p.effects; // предполагаем, что у игроков есть effects
    }
    return null;
  }
    ...
    try {
      ...
      if (n('ice') === 1 && n('fire') === 1 && len === 2) {
        if (hand === 'left') {
          ...
          // Проверка эффектов кастера через getPlayers
          let hasBurning = false, hasFreeze = false;
          for (const [id, p] of ctx.getPlayers()) {
            if (id === casterId && p.effects) {
              hasBurning = p.effects.has('burning');
              hasFreeze = p.effects.has('freeze');
              break;
            }
          }
          if (hasBurning) ctx.addEffect(casterId, 'fire_resist', 3, 0.5);
          if (hasFreeze) ctx.addEffect(casterId, 'ice_skin', 3, 1);
      ...
      if (n('shield') === 1 && n('air') === 1 && len === 2) {
        if (hand === 'left') pushNova(casterId, ox, oy, oz, 2);
        else {
      ...
      if (n('light')===1 && n('dark')===1 && n('beam')===1 && n('shield')===1 && len===4) {
        if (hand === 'left') castBeam(casterId, els, n, len, ox, oy, oz, dx, dy, dz);
        else {
      ...
      if (n('earth') === 5 && len === 5) {
        if (hand === 'left') ctx.stomp(casterId, ox, oz, 6);
        else stoneWall(ox, oz, yaw, 7);
        return;
      }
      ...
      if (n('ice') === 5 && len === 5) {
        if (hand === 'left') {
          ...
          if (nearest && nearestPos) stoneWall(nearestPos.x, nearestPos.z, yaw, 4);
        }
        return;
      }
      ...
      // ========== ПРАВАЯ РУКА – стандартные защиты ==========
      if (hand === 'right') {
      ...
      // ========== СТАНДАРТНЫЕ 5-ЭЛЕМЕНТНЫЕ (старые) ==========
      if (len === 5) {
